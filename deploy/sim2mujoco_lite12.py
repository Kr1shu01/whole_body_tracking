...
config_file = f"{LEGGED_GYM_ROOT_DIR}/deploy/config/lite12.yaml"
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running() and time.time() - start < simulation_duration:
            ...
            if counter % control_decimation == 0:
                ...
                pelvis_quat = data.qpos[3:7]
                anchor_quat_w = ref_motion["body_quat_w"][frame_idx, 0]
                motion_anchor_ori_b = compute_ori_b(pelvis_quat, anchor_quat_w, data.qpos[7:])

                obs[0:26]  = ref_motion["joint_pos"][frame_idx]
                obs[26:52] = ref_motion["joint_vel"][frame_idx]
                obs[52:58] = motion_anchor_ori_b
                # obs[64:67] = data.qvel[3:6]
                obs[58:61] = pelvis_ang_vel
                # obs[67:96] = convert_joint_order(data.qpos[7:])-default_pos_in_policy
                # obs[96:125] = convert_joint_order(data.qvel[6:])
                obs[61:87] = convert_joint_order(motor_cur_pos)-default_pos_in_policy
                obs[87:113] = convert_joint_order(motor_cur_vel)
                obs[113:139] = action
