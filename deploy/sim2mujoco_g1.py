...
# 默认顺序到实际顺序的索引映射
mujoco_to_policy_idx = [ 0, 6, 12, 1, 7, 13, 2, 8, 14,
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running() and time.time() - start < simulation_duration:
            ...
            if counter % control_decimation == 0:
                ...
                obs[0:29]  = ref_motion["joint_pos"][frame_idx]
                obs[29:58] = ref_motion["joint_vel"][frame_idx]
                obs[58:64] = motion_anchor_ori_b
                # obs[64:67] = data.qvel[3:6]
                obs[64:67] = pelvis_ang_vel
                # obs[67:96] = convert_joint_order(data.qpos[7:])-default_pos_in_policy
                # obs[96:125] = convert_joint_order(data.qvel[6:])
                obs[67:96] = convert_joint_order(motor_cur_pos)-default_pos_in_policy
                obs[96:125] = convert_joint_order(motor_cur_vel)
                obs[125:154] = action
